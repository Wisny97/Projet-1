...
def image_extension(image_link: str) -> str:
    path = urlparse(image_link).path
    _, ext = os.path.splitext(path)
    return ext or ".jpg"

def download_image(image_link: str, product_title: str, category_name: str) -> str:
    if not image_link:
        return ""

    category_dir = os.path.join("images", safe_filename(category_name))
    os.makedirs(category_dir, exist_ok=True)

    image_name = safe_filename(product_title)[:100] + image_extension(image_link)
    ...
    try:
        ...
        print(f"  ! Image non telechargee pour '{product_title}': {exc}")
        return ""
        ...
        with open(filepath, "w", newline="", encoding="utf-8") as f:
            ...
            for url in product_urls:
                try:
                    ...
                    download_image(data["image_url"], data["title"], data["category"])
                    writer.writerow(data)
                except requests.RequestException as exc:
                    print(f"  ! Produit ignore ({url}) : {exc}")
